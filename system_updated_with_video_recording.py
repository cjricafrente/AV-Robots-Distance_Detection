...
_video_path   = os.path.join("video_logs", f"run_{run_timestamp}.mp4")
_video_fourcc = cv2.VideoWriter_fourcc(*'mp4v')
video_writer  = cv2.VideoWriter(
    _video_path, _video_fourcc, 30.0, (TARGET_WIDTH, TARGET_HEIGHT)
)
print(f"  Video log : {_video_path}")
print(f"\n--- Starting Live Feed ---")
...
while not rospy.is_shutdown():
    ...
    # ----------------------------------------------------------
    # HUD — annotations drawn unconditionally so the recorded
    # video always contains bounding boxes and state text,
    # regardless of the HEADLESS flag.
    # ----------------------------------------------------------
    for d in all_detections:
        x1, y1, x2, y2 = d['box']
    ...
    # VIDEO RECORDING — write the fully-annotated frame every iteration.
    # Runs regardless of HEADLESS so recordings are always produced.
...
# =============================================================
cap.release()
video_writer.release()   # VIDEO RECORDING — flush and close the mp4 file cleanly
if not HEADLESS:
