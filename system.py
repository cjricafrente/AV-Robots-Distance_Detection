...
camera_tilt_deg = 20.0      # <--- ENTER YOUR TRUE TILT HERE
...
class ObjectTracker:
    ...
    def update(self, object_id, raw_distance, current_fps, current_fsm_state, fp_x):
        ...
        if object_id not in self.tracks:
            ...
            return raw_distance, 0.0, float('inf'), False
        ...
        # Lateral filtering (Left/Right movement)
        ...
        if current_fsm_state in [State.FOLLOW, State.SLOW]:
            dynamic_threshold = 0.20 
        else:
            dynamic_threshold = 0.02 
            
        # Object is dynamic if it moves fast toward the camera OR moves fast sideways
        if abs(new_speed) > dynamic_threshold or lat_speed_px > 40.0: 
            track['dynamic_memory'] = current_time
            
        is_dynamic = (current_time - track['dynamic_memory']) < 2.0
        
        ttc = float('inf')
        if is_dynamic and new_speed > 0.01:
            ttc = final_dist / new_speed 

        track['speed'] = new_speed
        track['last_time'] = current_time
        
        return final_dist, new_speed, ttc, is_dynamic
...
while True:
        ...
        if r.boxes.id is not None:
            ...
            for i, box in enumerate(boxes):
                ...
                if raw_dist != float('inf'):
                    dist, speed, ttc, is_dynamic = tracker.update(oid, raw_dist, current_fps, current_state, fp_x)
                    
                    detection_data = {
                        'id': oid, 'label': label_name, 'box': (x1, y1, x2, y2), 
                        'fp': (fp_x, fp_y), 'dist': dist, 'speed': speed, 
                        'ttc': ttc, 'is_dynamic': is_dynamic, 'center_x': fp_x
                    }
    ...
    if critical_obstacle:
        ...
        # Calculate RC Car Speed based on static obstacles while driving
        if current_state in [State.FOLLOW, State.SLOW] and not is_dynamic:
            rc_car_speed_cm_s = critical_obstacle['speed'] * 100
            if rc_car_speed_cm_s < 0: rc_car_speed_cm_s = 0.0 
        
        if is_dynamic:
            if ttc <= TTC_THRESHOLD or dist <= D_STOP:
                current_state = State.STOP_DYNAMIC
                action_text = "STOP (DYNAMIC)"
                hud_color = (0, 0, 255)
                global_action = "STOP"
                sys_logger.log_decision_event(frame_count, "TTC_Critical", "STOP", critical_obstacle['id'])
            else:
                current_state = State.FOLLOW
                action_text = "TRACKING DYNAMIC"
        else:
            if current_state in [State.FOLLOW, State.SLOW]:
                if dist <= D_STOP:
                    current_state = State.OBSERVE
                    state_start_time = time.time()
                    action_text = "OBSERVE (STATIC)"
                    hud_color = (0, 0, 255)
                    global_action = "STOP"
                    sys_logger.log_decision_event(frame_count, "Distance_Safety_Net", "OBSERVE", critical_obstacle['id'])
                elif dist <= D_SLOW:
                ...
                hud_color = (0, 0, 255)
                global_action = "STOP"
                if time.time() - state_start_time > TIME_OBSERVE:
