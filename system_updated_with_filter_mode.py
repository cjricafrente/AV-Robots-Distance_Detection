...
cap = cv2.VideoCapture('vid2.mp4') # 1, cv2.CAP_V4L2 kapag nasa raspi na
...
frame_delay_ms = int((1.0 / video_fps) * 1000)
print(f"  Video FPS: {video_fps:.1f} | Frame delay: {frame_delay_ms}ms")
...
prev_time               = time.time()
target_time             = time.time()  # For real-time video playback
...
while True:
    ...
    if not HEADLESS:
        ...
        # Real-time video playback: wait until target time
        current_time = time.time()
